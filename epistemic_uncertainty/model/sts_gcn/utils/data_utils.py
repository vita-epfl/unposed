...
def unNormalizeData(normalizedData, data_mean, data_std, dimensions_to_ignore, actions, one_hot):
    ...
    Args
      ...
      data_mean: vector of mean used to scale the data
...
def normalize_data(data, data_mean, data_std, dim_to_use, actions, one_hot):
    ...
    Args
      data: nx99 matrix with data to scale
      data_mean: vector of mean used to scale the data
      data_std: vector of standard deviation used to scale the data
...
def normalization_stats(completeData):
    ...
    Args
      completeData: nx99 matrix with data to scale
    Returns
      data_mean: vector of mean used to scale the data
      data_std: vector of standard deviation used to scale the data
      dimensions_to_ignore: vector with dimensions not used by the model
...
def rotmat2euler_torch(R):
    ...
    if len(idx_spec2) > 0:
        R_spec2 = R[idx_spec2, :, :]
        eul_spec2 = _move_to_device(torch.zeros(len(idx_spec2), 3).float())
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = _move_to_device(torch.zeros(len(idx_remain), 3).float())
