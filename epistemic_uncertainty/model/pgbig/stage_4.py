...
import math
from model.pgbig import BaseModel as BaseBlock
import numpy as np
...
class MultiStageModel(Module):
    def __init__(self, in_features, num_stages, d_model, kernel_size, drop_out, input_n, output_n, dct_n, cuda_idx):
    ...
    def forward(self, src, input_n=10, output_n=10, itera=1):
        ...
        print(dct_m.shape)
        dct_m = torch.from_numpy(dct_m).float().to('cuda:0')
        idct_m = torch.from_numpy(idct_m).float().to('cuda:0')
